...
<div id="viewproperties">
	...
	if ($min_price !== '-' && $max_price !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE rent BETWEEN ".$min_price." AND ".$max_price." ";
		}
		else
		{
			$where_clause .= "AND rent BETWEEN ".$min_price." AND ".$max_price." ";
		}
	}
	else if($min_price !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE rent >== ".$min_price." ";
		}
		else
		{
			$where_clause .= "AND rent <== ".$max_price." ";
		}
	}
	else if($max_price !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE rent <== ".$max_price." ";
		}
		else
		{
			$where_clause .= "AND rent <== ".$max_price." ";
	...
	{	
		if(empty($where_clause))
		{
			$where_clause .= "WHERE kitchen='".$kitchen."' ";
		}
		else
		{
			$where_clause .= "AND kitchen='".$kitchen."' ";
		}
	}
	if ($beds !== '-')
	{
	...
	if ($baths !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE baths=".$baths." ";
		}
		else
		{
			$where_clause .= "AND baths=".$baths." ";
		}
	}
	if ($driveway !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE driveway='".$driveway."' ";
		}
		else
		{
			$where_clause .= "AND driveway='".$driveway."' ";
		}
	}
	if ($garden !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE garden='".$garden."' ";
		}
		else
		{
			$where_clause .= "AND garden='".$garden."' ";
		}
	}
	if ($conservatory !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE conservatory='".$conservatory."' ";
		}
		else
		{
			$where_clause .= "AND conservatory='".$conservatory."' ";
		}
	}
	if ($receptions !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE receptions=".$receptions." ";
		}
		else
		{
			$where_clause .= "AND receptions=".$receptions." ";
		}
	}
	if ($type !== '-')
	{
	...
	if ($sub_type !== '-')
	{
		if(empty($where_clause))
		{
			$where_clause .= "WHERE sub_type='".$sub_type."' ";
		}
		else
		{
			$where_clause .= "AND sub_type='".$sub_type."' ";
		}
	}
	if ($age !== '-')
	{
